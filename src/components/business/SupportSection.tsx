...
const icon = (src: string, className = "h-8 w-9 sm:h-10 sm:w-11") => (
...
const programs: Program[] = [
  ...
  },
  {
    ...
    icon: (
      <span className="flex size-7 items-center justify-center rounded-[5px] border-2 border-[#3478F6] sm:size-9 sm:border-[2.5px]">
        {icon(
          "/business/support-keypad.svg",
          "h-[11px] w-[17px] sm:h-[14px] sm:w-[22px]",
        )}
      </span>
    ),
    ...
    <section id="support" className="scroll-mt-28 bg-white py-14 md:py-[100px]">
      <div className="mx-auto w-full max-w-[1160px] px-5 sm:px-6 md:px-10">
        ...
        {/* Cards — Figma: 240×220, 4-across, 40px column gap, 20px row gap.
            Mobile: 2-per-row with scaled-down padding + type (like the home
            app-features grid). */}
        <m.div
          initial="hidden"
          whileInView="visible"
          ...
          className="mt-10 grid grid-cols-2 gap-3 sm:gap-5 md:mt-14 lg:grid-cols-4 lg:gap-x-10 lg:gap-y-5"
        >
          {programs.map((p) => (
            <m.div
              key={p.title}
              variants={fadeUp}
              className={cn(
                "flex min-h-[170px] flex-col gap-3 rounded-[16px] border border-[#E9E9E9] p-4 backdrop-blur-[12px] sm:min-h-[200px] sm:gap-4 sm:rounded-[24px] sm:p-[30px] lg:min-h-[220px]",
                p.yellow ? "bg-[#FFFCDC]" : "bg-white",
              )}
            >
              <div className="flex h-8 items-center sm:h-10">{p.icon}</div>
              <div className="flex flex-col gap-2">
                <h3
                  className={cn(
                    "font-solar text-[16px] font-medium leading-[1.2] text-[#212121] sm:text-[20px]",
                ...
                {p.sub && (
                  <p className="text-[13px] leading-[1.5] text-[#6A6A6A] sm:text-[15px] md:text-[16px]">
                    {p.sub}
                  </p>
                )}
              </div>
